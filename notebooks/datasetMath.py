...
def load_math_data(split: str, data_dir: str = "data/math/raw_data") -> list:
    """
...
def take_sample_size_match_train_category_distribution(dataset, sample_size):
    # Get the distribution of categories in the train dataset
    train_data = load_math_data("train")
    train_category_distribution = {x["category"]: len([x for x in train_data if x["category"] == x["category"]]) for x in dataset}
    # Now sample from the dataset such that the distribution of categories is the same as the train dataset
    sampled_dataset = []
    for category in set(dataset["category"]):
        category_data = [x for x in dataset if x["category"] == category]
...
def create_math_dataset(validation_sample_size=100):
    """
    ...
    # Save the dataset
    output_dir = "data/math/train_val_test_splits_dataset"
...
def create_feedback_dataset(feedback_sample_size=100):
    """
    ...
    """
    # Load all data
    main_data = load_from_disk("data/math/raw_data/train_val_test_splits_dataset")
    
    # Filter for training data
    train_data = main_data["train"]
    
    # Create feedback dataset by sampling
    ...
    # Save the dataset
    output_dir = f"data/math/raw_data/train_val_test_splits_dataset_feedback_{feedback_sample_size}"
    os.makedirs(output_dir, exist_ok=True)
    dataset_dict.save_to_disk(output_dir)
    logger.info(f"Saved dataset to {output_dir}")
