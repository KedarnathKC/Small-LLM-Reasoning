...
# userContent += "Your final answer should be stated as  \"#### [answer]\" where [answer] is the final numerical answer to the problem.\n"
...
for i in tqdm(range(0, len(data_test["question"]), batch_size), desc="Processing questions"):
    ...
    with torch.no_grad():
        output = model.generate(**tokenized_inputs, max_new_tokens = 1024,pad_token_id=tokenizer.pad_token_id,do_sample=False)
...
output_file_name = f"../outputs/gsm8k/LLaMA8B/generated_outputs_test_new_prompt_lm_eval_harness_with_max_new_len_1024_{temp}_{top_p}_{top_k}.json" 
with open(output_file_name, "w") as f:
    json.dump(generated_outputs, f, indent=4)

score = get_score(data_eval_path,output_file_name)
print("SCORE of LLaMA 8B Model is: ",score)