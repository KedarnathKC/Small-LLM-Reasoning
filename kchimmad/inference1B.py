...
# Defaults to float32
model = AutoModelForCausalLM.from_pretrained(model_name, token=hf_token, cache_dir=cache_dir, device_map=device)
...
for i in tqdm(range(0, len(data_test["question"]), batch_size), desc="Processing questions"):
    ...
    for q in batch_questions:
        ...
        # ] 
        # chat = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=False, include_metadata=False)
        # inputs.append(chat)
