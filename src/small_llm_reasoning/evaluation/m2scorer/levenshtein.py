...
from small_llm_reasoning.evaluation.m2scorer.util import uniq
...
def batch_multi_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing= False, verbose=False, very_verbose=False):
    ...
    # Added by Kedarnath, to collect f1(beta) scores for each sentence
    score=[]

    # for candidate, source, golds_set in zip(candidates, sources, gold_edits):
    for candidate, source, golds_set in tqdm(zip(candidates, sources, gold_edits), total=len(candidates)):
        i = i + 1
...
def merge_graph(V1, V2, E1, E2, dist1, dist2, edits1, edits2):
    ...
    for k in list(dist2.keys()):
        if k not in list(dist.keys()):
            ...
            if dist[k] != dist2[k]:
                print("WARNING: merge_graph: distance does not match!", )
